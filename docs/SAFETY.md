...
| Data                                   | Boundary                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Webcam video, eye crops, and landmarks | Processed locally by MediaPipe, Peekr, and ONNX Runtime. Camera input is optional. Do not add analytics or upload frames or eye features.                                                                                                                                                                                                             |
...
| Coarse attention point                 | Optional normalized screen position from pointer or calibrated gaze input, rounded to two decimal places and sent with live intent-suggestion requests after consent. This is derived attention data, not a camera frame or raw eye feature.                                                                                                          |
...
| Exported gaze diagnostics              | Created only when the person chooses **Download numeric diagnostics**. Contains viewport, per-target numeric errors, collection counts, and training-only fit summaries. No images, video, landmarks, raw feature vectors, or fitted model weights. Nothing is automatically uploaded. Review these personal performance measurements before sharing. |
...
- The appearance model is the pretrained Peekr CNN, downloaded at setup with a pinned source revision and SHA-256 check. It runs locally alongside MediaPipe. Its predictions are neither calibrated probabilities nor proof of where someone intended to look. Model licensing and provenance are recorded in [third-party notices](../THIRD_PARTY_NOTICES.md) and the [gaze research review](GAZE_RESEARCH.md).
- **Calibrate gaze** opens untimed instructions. Collection begins only after **Start gaze calibration**. A rolling temporal window checks eye/head signal stability; visible landmarks alone are not treated as a steady fixation. Large excursions, repeated alternating fixations, stale/reused frames, and frame gaps do not accumulate valid calibration time.
- Nine targets train a personal mapping from 18 features: iris/head geometry, two neural outputs, and eye-box geometry. Whole-training-target cross-validation chooses a feature view and linear/quadratic mapping. Five independently collected check targets are excluded from fitting and selection. Four check positions are unseen locations; the fifth repeats the center with fresh observations.
- Acceptance still requires mean error ≤ 0.150 and 95th-percentile error ≤ 0.255 in normalized viewport distance. The check is not a clinical standard. Per-target average offset and jitter are reported separately; a consistent error must not be mislabeled as noisy tracking. A failed check exposes no active gaze calibration.
...
The upgraded model and synthetic tests do not establish better accuracy for a real person. The local camera pipeline does not send frames to Astra or use language-model reasoning to manufacture a gaze estimate. Human measurements, comfort, glasses and lighting effects, and assistive suitability still require direct evaluation with consent.
