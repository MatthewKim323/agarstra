...
| `POST /api/session`    | `{mode:"practice"                     | "astra",screenConsent:boolean,url?:string}`                                            | Opens fresh isolated browser, returns state |
| `POST /api/intent`     | `{goal:string}`                       | Starts bounded planning asynchronously, returns state                                  |
| `POST /api/candidates` | `{point?:{x:number,y:number}}`        | Fresh browser observation, then three suggestions and their source; no computer action |
